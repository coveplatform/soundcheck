...
const submitReviewSchema = z.object({
  ...
  engagementCurve: z.array(z.object({ seconds: z.number(), level: z.number().int().min(1).max(5) })).optional().nullable(),
  ...
  try {
    ...
    if (!bypassLimit) {
      ...
      if (reviewsTodayCount >= MAX_REVIEWS_PER_DAY) {
        return NextResponse.json(
          { error: `You've reached the daily limit of ${MAX_REVIEWS_PER_DAY} review${MAX_REVIEWS_PER_DAY === 1 ? "" : "s"}. Check back tomorrow!` },
          { status: 429 }
        );
      }
    }
        ...
        try {
          const updated = await tx.review.updateMany({
            ...
            data: {
              ...
              engagementCurve: data.engagementCurve ?? undefined,
