...
function buildPreviewHtml(type: string): string {
  ...
  switch (type) {
    ...
    case "weekly-digest": {
      ...
      const content = `
        ${headerImg}
        <h1 style="margin: 0 0 6px; font-size: 24px; font-weight: 800; color: ${COLORS.black}; line-height: 1.2;">Hey Kris — your week on MixReflect</h1>
        <p style="margin: 0 0 24px; font-size: 15px; color: ${COLORS.gray}; line-height: 1.6;">Here's what happened this week.</p>
    ...
    case "credits-nudge": {
      ...
      const content = `
        ${headerImg}
        <h1 style="margin: 0 0 8px; font-size: 24px; font-weight: 800; color: ${COLORS.black}; line-height: 1.2;">Hey Kris, you've got 5 credits doing nothing.</h1>
        <p style="margin: 0 0 24px; font-size: 15px; color: ${COLORS.gray}; line-height: 1.6;">You've got <strong style="color: ${COLORS.black};">5 credits</strong> sitting unused. Queue up "Summer Nights" and get real ears on it — each credit gets you one structured review back.</p>
        ...
        <p style="margin: 16px 0 0; font-size: 13px; color: ${COLORS.grayLight}; text-align: center;">Or <a href="${appUrl}/review" style="color: ${COLORS.gray}; font-weight: 600;">review a track</a> to earn even more.</p>
      `;
      return emailWrapper(content);
    }
    case "totd-digest": {
...
export async function POST(request: Request) {
  try {
    ...
    switch (type) {
      ...
      case "weekly-digest":
        await sendWeeklyDigestEmail({ to: recipientEmail, name: "Kris", creditsEarned: 3, reviewsReceived: 2, genreTrackCount: 7, trackTitle: "Summer Nights", trackId: "test-id" });
        break;
      case "credits-nudge":
        await sendCreditsNudgeEmail({ to: recipientEmail, name: "Kris", credits: 5, trackTitle: "Summer Nights", trackId: "test-id" });
        break;
      case "totd-digest":
        await sendTotdWeeklyEmail({
          ...
          weekLabel: "May 19–25",
          picks: [
            { title: "Low Tide", artistName: "Niko Vale", genre: "Lo-fi", editorNote: "There's a stillness to this one that feels deliberate — worth your full attention." },
            { title: "Neon Pulse", artistName: "Maya Kim", genre: "Synthwave", editorNote: "Somewhere between John Carpenter and Kavinsky, but neither. Four notes doing a lot of work." },
            { title: "Rough Draft", artistName: "Theo James", genre: "Indie", editorNote: "Caught mid-thought, with a rawness the production leans into. The hook is better than the artist probably knows." },
          ],
        });
        break;
      default:
